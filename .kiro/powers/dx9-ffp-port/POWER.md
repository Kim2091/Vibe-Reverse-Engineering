...
- **Concatenated WVP/VP instead of separate matrices**: **#1 Remix porting mistake.** If CTAB shows "WorldViewProj" or only one matrix register per draw, you have this problem. Hook individual W/V/P before concatenation.
- **Matrices look wrong**: FFP `SetTransform` expects row-major. Proxy transposes. If game stores row-major VS constants (uncommon), remove transpose in `apply_transforms()`.
- **Everything is white/black**: Albedo on stage 1+, not 0. Set `AlbedoStage` in INI.
- **Some objects missing**: Check NORMAL in vertex decl. Check `ffp.view_proj_valid()` at draw time.
- **Skinned meshes invisible**: Enable `[Skinning] Enabled=1`. Check log for bone count.
- **Game crashes on startup**: Set `[Remix] Enabled=0` to test without Remix. Check `WINDOW_CLASS_NAME`.
- **Geometry at origin**: World matrix register mapping wrong. Trace VS constant writes.
- **World shifts after skinned draws**: `WORLDMATRIX(0)` clobbered by bone[0]. Proxy tracks `world_dirty_`. Check bone register overlap.
- **ImGui overlay not appearing**: Press F4. Check `WINDOW_CLASS_NAME` correct and DirectInput hook conflicts.
